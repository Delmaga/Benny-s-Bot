...
# Stockage simple en mémoire (à remplacer par DB en prod)
SAY_MESSAGES = {}
    ...
    async def on_submit(self, interaction: discord.Interaction):
        if self.message_id:
            if self.message_id not in SAY_MESSAGES:
                await interaction.response.send_message("`❌ Message non éditable.`", ephemeral=True)
                return
...
class SayCog(commands.Cog):
    ...
    async def editsay(self, interaction: discord.Interaction, message_id: str):
        ...
        if msg_id not in SAY_MESSAGES:
            await interaction.response.send_message("`❌ Ce message n'est pas éditable.`", ephemeral=True)
            return
        original = await interaction.channel.fetch_message(msg_id)
        await interaction.response.send_modal(SayModal(message_id=msg_id, original=original.content))
