...
class SayModal(ui.Modal, title="`Composer un message`"):
    content = ui.TextInput(
        ...
        placeholder="*italique*, _souligné_, ``code``, @mention, espaces, etc.",
        required=True,
        max_length=2000
    )
...
class SayCog(commands.Cog):
    ...
    @app_commands.command(name="editsay", description="Modifier un message envoyé via /say")
    async def editsay(self, interaction: discord.Interaction, message_id: str):
        ...
        if msg_id not in SAY_MESSAGES:
            await interaction.response.send_message("`❌ Ce message n'a pas été envoyé via /say.`", ephemeral=True)
            return
        original_content = ""
        try:
